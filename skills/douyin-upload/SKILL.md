...
| 本地团购图文 | `sau douyin upload-note ... --location "<POI>" --headed` | 发布图文并挂国内 POI 位置 |
| 普通图文上传 | `sau douyin upload-note ...` | 上传并发布抖音图文 |

元数据约定：

- 视频使用 `title + desc + tags`
- 本地团购视频/图文额外使用 `location`，建议传完整 POI 名称
...
- 使用 `sau douyin upload-note ... --location "<POI>"` 发布本地团购图文并挂 POI
- 使用 `sau douyin upload-note ...` 上传普通抖音图文
...
- 当用户要发布本地团购图文、卡券图文、挂地理位置或 POI 时，使用 `upload-note --location "<POI>" --headed`
- 当用户要发布普通图文时，使用 `upload-note`
...
- 本地团购视频/图文首次跑新账号时优先显式传 `--headed`，方便确认平台弹窗、封面诊断和位置候选
- `--location` 要写完整商场/门店 POI，例如 `合肥滨湖银泰百货`
- 发布成功必须以 CLI 成功退出、页面出现成功状态或进入作品管理页为准；不能只看“位置已选择”
- 如果点击发布后出现短信验证码，这是抖音发布风控，不是 cookie 失效；输入验证码后继续，未输入时不要声称已发布
...
3. 在 `添加标签` 选择 `位置`
4. 选择 `带货模式`
5. 搜索并选择 `--location` 对应 POI
6. 校验位置字段保留成功
7. 点击发布并等待作品管理页或成功状态
8. 如触发短信验证码，等待验证码输入后继续
